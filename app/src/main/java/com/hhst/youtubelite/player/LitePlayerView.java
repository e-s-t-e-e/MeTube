...
public class LitePlayerView extends PlayerView {
	...
	@Nullable
	private MiniDismissListener miniDismissListener;

	/** Reports mini-player drag lifecycle so a host can show a drag-to-dismiss target. */
	public interface MiniDismissListener {
		void onDragStart();

		void onDrag(float rawX, float rawY);

		/** @return true if the drop dismissed the mini-player (host handled close); skip snap-back. */
		boolean onDrop(float rawX, float rawY);
	...
	public void setMiniDismissListener(@Nullable MiniDismissListener listener) {
		miniDismissListener = listener;
	}

	/** Dismisses the mini-player via the configured close action, if any. */
	public void dismissMiniPlayer() {
		if (onMiniPlayerClose != null) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE -> {
				...
				if (!miniPlayerDragging && exceedsTouchSlop(deltaX, deltaY)) {
					...
					clearMiniPlayerPendingTapTarget();
					if (miniDismissListener != null) {
						miniDismissListener.onDragStart();
					}
				}
				if (miniPlayerDragging) {
					...
					if (miniDismissListener != null) {
						miniDismissListener.onDrag(event.getRawX(), event.getRawY());
					}
				}
				return true;
			}
			...
			case MotionEvent.ACTION_UP -> {
				...
				if (wasDragging) {
					if (miniDismissListener != null
									&& miniDismissListener.onDrop(event.getRawX(), event.getRawY())) {
						return true; // dismissed; skip snap-back
					}
					snapMini();
					return true;
				}
			...
			case MotionEvent.ACTION_CANCEL -> {
				...
				if (wasDragging) {
					if (miniDismissListener != null) {
						miniDismissListener.onDragCancel();
					}
					snapMini();
				}
				return true;
