...
public class Controller {
	...
	private void updateLevelIndicatorMargins() {
		// Removed programmatic margin override to keep horizontal gesture UI at the top center.
	}
	...
	private void setupListeners() {
		...
		playerView.setOnTouchListener((v, ev) -> {
			...
			if (!handled && isFullscreen() && !state.isLocked()) zoomListener.onTouch(ev);
	...
	/** Whether the player is in the locked (gestures-frozen) state. */
	public boolean isLocked() {
		return state.isLocked();
	}

	/** Minimizes the current watch page into the in-app mini-player (same path as back-navigation). */
